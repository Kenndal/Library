...
public class BibliotekaGUI implements Obserwator {
    ...
    public BibliotekaGUI(Biblioteka biblioteka) {
        ...
        dodajCzytelnikaButton.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                biblioteka.dodaj_czytelnika(textFieldImie.getText(),textFieldNazwisko.getText(),textFieldData.getText(),textFieldTelefon.getText());
    ...
    private void reflashTableCzytelnicy(){
        ...
        for(int i=0;i<biblioteka.getCzytelnicy().size();i++){
            model.addRow(new Object[]{biblioteka.getCzytelnicy().get(i).getImie(),biblioteka.getCzytelnicy().get(i).getNazwisko(),biblioteka.getCzytelnicy().get(i).getIndeks_czytelnika(),biblioteka.getCzytelnicy().get(i).getPosiadanieKsiazek()});
        }
    }
    ...
    private void reflashDodajKsiazki(Ksiazka ksiazka){
        //dodawanie ksiazki
        DefaultTableModel model = (DefaultTableModel) tableKsiazki.getModel();
        model.addRow(new Object[]{ksiazka.getNazwa(), ksiazka.getAutor(),ksiazka.getGatunek(),"true",ksiazka.getIndeks()});
    }
    // dodawanie nowego
    private void reflashtableCzytelnik(Czytelnik czytelnik){
        //dodawanie czytelnika
        DefaultTableModel model = (DefaultTableModel) tableCzytelnicy.getModel();
        model.addRow(new Object[]{czytelnik.getImie(), czytelnik.getNazwisko(),czytelnik.getIndeks_czytelnika(),false});
